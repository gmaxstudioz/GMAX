...
import { UpdateBookingSchema, UpdateBookingInput } from "@/lib/schemas/booking";
...
export function UpdateBookingDialog({ bookingId, currentData, clients, services, members }: UpdateBookingDialogProps) {
    ...
    const form = useForm<UpdateBookingInput>({
        ...
        defaultValues: {
            ...
            bookingStatus: currentData.bookingStatus as "PENDING" | "CONFIRMED" | "COMPLETED" | "CANCELLED",
            paymentStatus: currentData.paymentStatus as "PENDING" | "PAID" | "PARTIALLY_PAID" | "CANCELLED",
            deliveryStatus: currentData.deliveryStatus as "PENDING" | "DELIVERED" | "CANCELLED",
                                ...
                                <span className={selectedService ? "" : "text-muted-foreground"}>
                                    {selectedService ? (
                                        <span className="flex items-center gap-2">
                                            ...
                                            <Badge variant="outline" className="text-[10px] capitalize">{selectedService.isAddon ? "Addon" : "Service"}</Badge>
                                        </span>
                                    ) : "Choose a service..."}
                                </span>
                                <ChevronDown className="h-4 w-4 opacity-50" />
