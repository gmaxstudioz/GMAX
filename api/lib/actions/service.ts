...
export async function createCategory(data: CategoryPayload) {
  ...
  try {
    ...
    const member = await prisma.member.findFirst({ where: { userId: session.user.id, studioId: parsed.data.studioId! } });
    ...
    revalidatePath(`/dashboard/studios/[slug]`, "page");
...
export async function updateCategory(id: string, data: CategoryPayload) {
  ...
  try {
    ...
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session?.user) return { status: "error", message: "Unauthorized" };
    const member = await prisma.member.findFirst({ where: { userId: session.user.id, studioId: existing.studioId } });
    if (!member) return { status: "error", message: "Unauthorized access to studio" };
...
export async function createService(data: ServicePayload) {
  ...
  try {
    ...
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session?.user) return { status: "error", message: "Unauthorized" };
    const member = await prisma.member.findFirst({ where: { userId: session.user.id, studioId: category.studioId } });
    if (!member) return { status: "error", message: "Unauthorized access to studio" };
...
export async function updateService(id: string, data: ServicePayload) {
  ...
  try {
    ...
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session?.user) return { status: "error", message: "Unauthorized" };
    const member = await prisma.member.findFirst({ where: { userId: session.user.id, studioId: existing.category.studioId } });
    if (!member) return { status: "error", message: "Unauthorized access to studio" };
